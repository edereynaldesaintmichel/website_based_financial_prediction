...
2. **Clone repo and run setup** on the remote:
   ...
   cd website_based_financial_prediction && bash training_pipeline/setup.sh
   ```
...
4. **Upload local training data** from `training_data/bucketed/` to remote `/workspace/data/bucketed/`:
   - Wipe the remote directory first (`rm -rf /workspace/data/bucketed/*`)
   - Then rsync all `.pt` files up:
     ```
     rsync -avz --progress -e "ssh -p <PORT>" training_data/bucketed/ root@<HOST>:/workspace/data/bucketed/
     ```
...
6. **Give the user the training command** to run in their remote terminal:
   ```
   cd /workspace/website_based_financial_prediction && python -m training_pipeline.train_mlm_full_baseline --data_dir /workspace/data/bucketed --num_workers 4 --tokens_per_batch <BATCH> --lr <LR> --number_lr <NUMBER_LR> --dtype bf16 --device cuda --compile
   ```
